...
// Offsets Updated for Game Version v1.1.4
#define WORLD_OFFSET 0x51b5578		//(OLD: 0x50FA638 v1.1.2 | 0x50b3468 v1.1.1 | 0x513b898 v1.1.3)
#define OBJECTS_OFFSET 0x50f360	//(OLD: 0x503D000 v1.1.2 | 0x4ff6990 v1.1.1 | 0x507e290 v1.1.3)
#define NAME_OFFSET 0x5057e40		//(OLD: 0x5038840 v1.1.2 | 0x4ff21d0 v1.1.1 | 0x5079ae0 v1.1.3)
...
void ReadData()
{
	// We only want to set the module the first time ReadData() is called.
	// So we set the FirstRun boolean to false the first time ReadData() is called.
	if (FirstRun)	
	{
...
// Treasure X,Y,Z Location (Vector3)
std::vector<Vector3> XMarksTheSpot;


bool get_TreasureMap(DWORD_PTR _PTR, std::string * MapTexturePath, std::vector<Vector2> * Marks) {
	try {
		DWORD_PTR Name_PTR = mem.Read<DWORD_PTR>(_PTR + 0x878);			// Offset for v1.1.3
		DWORD_PTR Marks_PTR = mem.Read<DWORD_PTR>(_PTR + 0x8B8);		// Offset for v1.1.3
		__int32 Marks_Cout = mem.Read<__int32>(_PTR + 0x8B8 + 0x8);		// Offset for v1.1.3
